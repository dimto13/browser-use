...
async def test_cdp_client_headers_passed_on_connect():
	...
	with patch('browser_use.browser.session.BrowserSession.get_or_create_cdp_session', new_callable=AsyncMock):
		with patch('browser_use.browser.session.CDPClient') as mock_cdp_client_class:
			...
			mock_cdp_client.send.Target = AsyncMock()
			mock_cdp_client.send.Target.setAutoAttach = AsyncMock()
			mock_cdp_client.send.Target.setDiscoverTargets = AsyncMock()
			mock_cdp_client.send.Target.getTargets = AsyncMock(return_value={'targetInfos': []})
			mock_cdp_client.send.Target.attachToTarget = AsyncMock(return_value={'sessionId': 'test-session-id'})
